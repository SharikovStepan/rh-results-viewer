...
if (language == 'ru') {
	...
	textStrings = {
		...
		pilotsTab: {
			...
			time: 'Time',
			bestConsecutive: `Best ${consecutivesCount} consecutive`,
		...
		leaderboardTab: {
			lap: 'Lap',
			consecutive: `${consecutivesCount} Consecutive`,
			totalLaps: 'Total laps',
			average: 'Average',
			name: 'Name',
			round: 'Round',
...
function getRound(roundNum, heatNum) {
	...
	roundPilotsLaps.forEach(function (pilot, index) {
		if (index != 0) {
			...
			for (let i = 1; i <= consecutivesLenghtFinals; i++) {				//Добавляем 3 круга (3 через consecutivesLenghtFinals)
				try {
